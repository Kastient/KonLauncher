...
const UI_STRINGS = {
  en: {
    ...
    dropJarToInstall: 'Drop .jar mod file here to import',
    dropJarOnly: 'Only .jar mod files are supported here.',
    failedImportLocalMod: 'Failed to import local mod file(s).',
    importingLocalMods: 'Importing local mod file(s)...',
  ...
  ru: {
    ...
    dropJarToInstall: 'Отпусти .jar мод здесь для импорта',
    dropJarOnly: 'Здесь поддерживаются только .jar моды.',
    failedImportLocalMod: 'Не удалось импортировать локальные моды.',
    importingLocalMods: 'Импорт локальных модов...',
...
const isJarFileName = (value) => /\.jar$/i.test(String(value || '').trim());
...
export default function App({ headerCenterSlot = null }) {
  ...
  const [instanceFileDropOverId, setInstanceFileDropOverId] = useState(null);
  const [instanceFileDropBusyId, setInstanceFileDropBusyId] = useState(null);
  ...
  useEffect(() => {
    if (draggingInstanceId) {
      setInstanceFileDropOverId(null);
    }
  }, [draggingInstanceId]);
  ...
  const importDroppedModsToInstance = useCallback(
    async (instance, droppedFiles) => {
      if (!instance || !window.launcherMinecraft?.importLocalContent) return;
      if (instance.installState !== 'installed' || !String(instance.installPath || '').trim()) {
        notifyError(t('installFirstForMods'));
        return;
      }

      const files = Array.isArray(droppedFiles) ? droppedFiles : [];
      const jarFiles = files.filter((file) => isJarFileName(file?.name));
      if (!jarFiles.length) {
        notifyError(t('dropJarOnly'));
        return;
      }

      const instanceId = String(instance.id);
      setInstanceFileDropBusyId(instanceId);
      const failures = [];
      ...
      try {
        for (const file of jarFiles) {
          ...
          if (!sourcePath) {
            if (typeof file?.arrayBuffer === 'function') {
              try {
                inlineBuffer = await file.arrayBuffer();
              } catch {
                inlineBuffer = null;
              }
            }
            if (!inlineBuffer) {
              failures.push(`${file?.name || 'unknown.jar'}: file path unavailable`);
              continue;
            }
          }

          const latestInstance = instancesRef.current.find((item) => String(item.id) === instanceId) || instance;
          const knownItems = resolveContentListByType(latestInstance, 'mod');
          const result = await window.launcherMinecraft.importLocalContent({
            instanceId,
            installPath: latestInstance.installPath,
            contentType: 'mod',
            sourcePath,
            sourceName: file?.name || 'mod.jar',
            buffer: inlineBuffer,
            items: knownItems
          });

          if (!result?.ok || !result?.data) {
            failures.push(`${file?.name || 'unknown.jar'}: ${result?.error?.message || t('failedImportLocalMod')}`);
            continue;
          }

          importedAny = true;
          const importedItem = result.data;
          mergeInstanceContent(instanceId, 'mod', (prevList) => {
            const importedPath = String(importedItem.filePath || '').toLowerCase();
            const importedProjectId = String(importedItem.projectId || importedItem.id || '').trim();
            const importedFilename = String(importedItem.filename || '').trim().toLowerCase();
            const existingIndex = prevList.findIndex((entry) => {
              if (importedPath && String(entry?.filePath || '').toLowerCase() === importedPath) return true;
              if (importedProjectId && String(entry?.projectId || entry?.id || '').trim() === importedProjectId) return true;
              return importedFilename && String(entry?.filename || '').trim().toLowerCase() === importedFilename;
            });

            if (existingIndex === -1) {
              return [...prevList, importedItem];
            }
            const next = [...prevList];
            next[existingIndex] = {
              ...next[existingIndex],
        ...
        setInstanceFileDropBusyId(null);
      }

      if (failures.length) {
        notifyError(t('failedImportLocalMod'), 'LOCAL_MOD_IMPORT_FAILED', failures.join('\n'));
      }
      if (!importedAny) return;
      setEditingInstanceId(instanceId);
      await syncInstanceContentFromDisk(instanceId, 'mod');
    },
    [mergeInstanceContent, notifyError, syncInstanceContentFromDisk, t]
  );
              ...
              (instances.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                  ...
                  {instances.map((inst) => {
                    ...
                    const isFileDropOver = instanceFileDropOverId === instanceId;
                    const isFileDropBusy = instanceFileDropBusyId === instanceId;
                    const dropStatusText = isFileDropBusy ? t('importingLocalMods') : isFileDropOver ? t('dropJarToInstall') : '';
                    ...
                    return (
                      <div
                        ...
                        onDragEnter={(event) => {
                          ...
                          if (!hasFiles || draggingInstanceId) return;
                          event.preventDefault();
                          setInstanceFileDropOverId(instanceId);
                        }}
                        onDragOver={(event) => {
                          const hasFiles = Array.from(event.dataTransfer?.types || []).includes('Files');
                          if (!hasFiles || draggingInstanceId) return;
                        ...
                        onDragLeave={(event) => {
                          ...
                          setInstanceFileDropOverId((current) => (current === instanceId ? null : current));
                        }}
                        onDrop={(event) => {
                          const hasFiles = Array.from(event.dataTransfer?.types || []).includes('Files');
                          if (!hasFiles || draggingInstanceId) return;
                          ...
                          void importDroppedModsToInstance(inst, droppedFiles);
                        }}
                        className={`group rounded-[2.5rem] border bg-zinc-900/30 p-7 transition-all ${
                          isDragging
                            ...
                            : isFileDropOver
                              ? 'border-emerald-300/80 bg-emerald-500/[0.08] shadow-[0_0_0_1px_rgba(110,231,183,0.45)]'
                              : isDragOver
                              ? 'border-emerald-300/70 shadow-[0_0_0_1px_rgba(52,211,153,0.42)]'
                              : 'border-white/5 hover:border-green-500/30'
                        } ${dragEnabled ? 'cursor-grab' : ''} ${isFileDropBusy ? 'border-emerald-300/70' : ''}`}
                        style={
                          isDragging
                            ? {
                                position: 'fixed',
                      ...
                      >
                        ...
                        {dropStatusText ? <p className="mb-4 text-[10px] font-bold text-emerald-300">{dropStatusText}</p> : null}
