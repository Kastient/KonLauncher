...
public final class MCEF {
    ...
    private static final String FALLBACK_JAVA_CEF_COMMIT = "eaeb3d4370aa3526ee237ad1981ad59af3de4dd1";
    ...
    public static String getJavaCefCommit() throws IOException {
        ...
        String propertyCommit = normalizeCommit(System.getProperty("mcef.java.cef.commit"));
        if (propertyCommit != null) {
            return propertyCommit;
        }
        ...
        resources.asIterator().forEachRemaining(resource -> {
            ...
            try {
                ...
                if (properties.containsKey("java-cef-commit")) {
                    String manifestCommit = normalizeCommit(properties.getProperty("java-cef-commit"));
                    if (manifestCommit != null) {
                        commits.put(resource.getFile(), manifestCommit);
                    }
                }
            } catch (IOException e) {
                e.printStackTrace();
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                String gitCommit = normalizeCommit(parts[0].replace("+", "").replace("-", ""));
                if (gitCommit != null) {
                    return gitCommit;
                }
            }
        } catch (IOException e) {
            MCEF.getLogger().warn("Could not resolve java-cef commit from git. Falling back to {}", FALLBACK_JAVA_CEF_COMMIT);
        }

        MCEF.getLogger().warn("Could not resolve java-cef commit automatically. Falling back to {}", FALLBACK_JAVA_CEF_COMMIT);
        return FALLBACK_JAVA_CEF_COMMIT;
    }
